...
#include <type_traits> // 包含 std::conditional_t 等类型特性
#include <iterator> // 包含 std::forward_iterator_tag 等迭代器类别
...
public:
    ...
    HashMapIterator(HashMap* map, NodeType* node) 
        : current(node), buckets(&(map->_buckets_array)) {
        // 查找节点所在的桶索引
        bucketIndex = findBucketIndex(map, node);
    }

    // 辅助函数，用于找到节点所在的桶索引
    size_t findBucketIndex(HashMap* map, NodeType* node) {
