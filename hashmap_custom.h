...
public:
    ...
    *      map.insert(val);
    */
    using value_type = std::pair<const K, M>;//定义键值对
     ...
     */
    using iterator = HashMapIterator<HashMap, false>;//定义迭代器类型
     ...
     */
    using const_iterator = HashMapIterator<HashMap, true>;//定义常量迭代器类型
     ...
     */
    friend class HashMapIterator<HashMap, false>;//友元声明
    ...
    */
    explicit HashMap(size_t bucket_count, const H& hash = H()){
        _buckets_array = std::vector<node*>(bucket_count,nullptr);
        _hash_function = hash;
    }//带参构造
    ...
    * Complexity: O(1) (inlined because function is short)
    */
    inline bool empty(){return _size==0;}//检测空
    ...
    */
    inline float load_factor(){//负载因子
        return static_cast<float>(_size)/_buckets_array.size();
    }
    ...
    */
    inline size_t bucket_count() const{//桶数
    ...
    */
    bool contains(const K& key){//判断是否含有特定键
        size_t index = _hash_function(key)%_buckets_array.size();
        //n is linkedList node  n1->n2->n3->nullptr     n1: k-v pair
        node* n = _buckets_array.at(index);
    ...
    */
    M& at(const K& key){
        ...
        //eg. abc(hash = 111)  cde(hash=111) add abc but find cde, should throw exception
    ...
    */
     ...
     * Complexity: O(1) amortized average case, O(N) worst case, N = number of elements
     */
    // 在哈希表中查找给定键的元素，并返回一个迭代器指向该元素。
    // 如果没有找到元素，则返回一个指向 end() 的迭代器。
    ...
    */
    ...
    std::pair<iterator, bool> insert(const value_type& value){
        ...
        new_node->next = _buckets_array[index]; // 新节点指向当前桶的头节点
        _buckets_array[index] = new_node;       // 更新桶的头节点为新节点
        _size++;                                // 增加元素计数
        // 返回指向新插入键值对的迭代器和true
    ...
    */
    // 删除指定键的键值对，如果键存在则返回 true，否则返回 false
    bool erase(const K& key){
		...
		while (current != nullptr) {
			if (current->value.first == key) {
				if (prev == nullptr) {
					_buckets_array[index] = current->next;
				} else {
    ...
    */
    iterator erase(const_iterator pos){
       if (pos == end()) {
		...
		size_t index = _hash_function(pos->first) % _buckets_array.size();
		node* current = _buckets_array[index];
		node* prev = nullptr;
		...
		delete current;
		_size--;
		return iterator(this, prev ? prev->next : _buckets_array[index]); 
    }
    ...
    */
    ...
    void rehash(size_t new_buckets){
            if (new_buckets == 0) {
                throw std::out_of_range("new_buckets cannot be zero");
        }
        ...
        for (auto& bucket : _buckets_array) {
            ...
            while (current != nullptr) {
                ...
                size_t index = _hash_function(current->value.first) % new_buckets;
                current->next = new_buckets_array[index];
                new_buckets_array[index] = current;
                current = next;
                _size++;
            }
        }
    ...
    iterator begin(){
        ...
        for (size_t i = 0; i < _buckets_array.size(); ++i) {
            if (_buckets_array[i] != nullptr) {
                // 返回指向该桶头节点的迭代器
                return iterator(this, _buckets_array[i]);
            }
        }
        // 如果所有桶都为空，返回end()迭代器
        return end();
    ...
    const_iterator begin() const{
        ...
        for (size_t i = 0; i < _buckets_array.size(); ++i) {
           if (_buckets_array[i] != nullptr) {
            // 返回指向该桶头节点的常量迭代器
                return const_iterator(this, _buckets_array[i]);
            }
        }
        // 如果所有桶都为空，返回end()常量迭代器
        return end();
    }
    ...
    */
    ...
    M& operator[](const K& key){
        ...
        // 如果键不存在，创建新节点
        node* new_node = new node(value_type(key, M()));
        new_node->next = _buckets_array[index]; // 新节点指向当前桶的头节点
        _buckets_array[index] = new_node;       // 更新桶的头节点为新节点
        _size++;                                // 增加元素计数
        // 返回新节点的映射值
